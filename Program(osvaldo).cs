// var sair = "1";
// while (sair != "0")
// {
//     Console.WriteLine("Oiiii - " + sair);
//     sair = Console.ReadLine();
// }

// for (var i=10; i < 30; i++)
//     Console.WriteLine(i);


// var sair = "0";
// do
// {
//     Console.WriteLine("Oiiii - " + sair);
//     sair = Console.ReadLine();

// } while (sair != "0");

// string[] lista = new string[]{ "Danilo", "Alexandra", "Liah" };
// foreach (var item in lista)
// {
//     Console.WriteLine(item);
// }

// while (true)
// {
//     Console.WriteLine("Oiiii - ");
//     var sair = Console.ReadLine();
//     if(sair == "0") break;
// }


// while (true)
// {
//     Console.WriteLine("Digite um numero");
//     var numero = Console.ReadLine();
//     if(numero == "0") continue;

//     Console.WriteLine($"Você digitou um numero diferente de 0 {numero}");
// }


// string[] lista1 = new string[3];
// lista1[0] = "primeira";
// lista1[1] = "segunda";
// lista1[2] = "terceira";

// string[] lista2 = new string[]{ "Danilo", "Alexandra", "Liah" };

// List<string> lista3 = new List<string>();
// lista3.Add("Leandro");
// var lista4 = lista3.ToArray();

// foreach(var item in lista3)
// {
//     Console.WriteLine("Item: " + item);
// }



// int x = 0;
// while(x >= 0 && x <= 100) // E
// {
//     Console.WriteLine("digite o valor de x");
//     int.TryParse(Console.ReadLine(), out x);

//     Console.WriteLine("o  novo valor de x é: " + x);
// }


// int x = 0;
// while(!(x >= 0) || x <= 100) // OU
// {
//     Console.WriteLine("digite o valor de x");
//     int.TryParse(Console.ReadLine(), out x);

//     Console.WriteLine("o  novo valor de x é: " + x);
// }


using console_app.Models;
using console_app.Servicos;

while(true)
{
    Console.Clear();
    Console.WriteLine("Digite uma das opções abaixo:");
    Console.WriteLine("1 - Cadastrar Cliente");
    Console.WriteLine("2 - Listar Cliente");
    Console.WriteLine("3 - Editar Cliente");
    Console.WriteLine("4 - Pesquisar Cliente");
    Console.WriteLine("5 - Apagar Cliente");

    Console.WriteLine("6 - Sair");

    var opcao = Console.ReadLine();
    var sair = false;

    switch(opcao)
    {
        case "1":
            Console.Clear();
            Console.WriteLine("Digite o nome do cliente");
            var nome = Console.ReadLine();
            Console.WriteLine("Digite o telefone do cliente");
            var telefone = Console.ReadLine();

            Cliente cliente = new Cliente() 
            { 
                Nome = nome,
                Telefone = telefone 
            };
            PostgresPersistencia.Incluir(cliente);
            // var clientes = JsonPersistencia.Lista("clientes.json");
            // clientes.Add(cliente);
            // JsonPersistencia.Salvar("clientes.json", clientes);

            //var clientes = CsvPersistencia.Lista("clientes.csv");
            //clientes.Add(cliente);
            //CsvPersistencia.Salvar("clientes.csv", clientes);

            Console.WriteLine("Cliente cadastrado com sucesso ...");
            Thread.Sleep(1000);
            break;
        case "2":
            Console.WriteLine("=== Lista de clientes =====");
            // var clientesArquivo = JsonPersistencia.Lista("clientes.json");
            //var clientesArquivo = CsvPersistencia.Lista("clientes.csv");
            List<Cliente> clientes = PostgresPersistencia.Buscar();
            foreach(var cli in clientesArquivo)
            {
                Console.WriteLine($"Nome: {cli.Nome}");
                Console.WriteLine($"Telefone: {cli.Telefone}");
                Console.WriteLine("------------------------------");
            }

            Console.WriteLine("Pressione Enter para continuar ...");
            Console.ReadKey();
            break;
        case "3":
                Console.Clear();
                Console.WriteLine("=== Edicão de Clientes ===");
                var clientes = PostgresPersistencia.Buscar();
                foreach(var cli in clientes)
                {
                    Console.WriteLine($"ID: {cli.Id}");
                    Console.WriteLine($"Nome: {cli.Nome}");
                    Console.WriteLine($"Telefone: {cli.Telefone}");
                    Console.WriteLine("------------------------------");
                }
                
                Console.WriteLine("Digite o ID do cliente que deseja atualizar:");
                var id = Console.ReadLine();
                var clienteSelecionado = clientes.FirstOrDefault(c => c.Id == id);
                if(clienteSelecionado == null)
                {
                    Console.WriteLine("ID de cliente inválido.");
                    Thread.Sleep(1000);
                    break;
                }
                
                Console.WriteLine("Digite o novo nome do cliente:");
                var nome = Console.ReadLine();
                Console.WriteLine("Digite o novo telefone do cliente:");
                var telefone = Console.ReadLine();
                
                clienteSelecionado.Nome = nome;
                clienteSelecionado.Telefone = telefone;
                PostgresPersistencia.Atualizar(clienteSelecionado);
                
                Console.WriteLine("Cliente atualizado com sucesso ...");
                Thread.Sleep(1000);
        break;    
        case "4":
            Console.Clear();
        Console.WriteLine("Digite o nome do cliente a ser pesquisado:");
        var nomePesquisa = Console.ReadLine();
        var clientesPesquisados = clientes.Where(cli => cli.Nome.Contains(nomePesquisa)).ToList();
        
        if(clientesPesquisados.Count == 0)
        {
            Console.WriteLine("Nenhum cliente encontrado.");
        }
        else
        {
            Console.WriteLine($"Foram encontrados {clientesPesquisados.Count} cliente(s) com o nome '{nomePesquisa}':");
            foreach(var cli in clientesPesquisados)
            {
                Console.WriteLine($"ID: {cli.Id}");
                Console.WriteLine($"Nome: {cli.Nome}");
                Console.WriteLine($"Telefone: {cli.Telefone}");
                Console.WriteLine("------------------------------");
            }
        }
        
        Console.WriteLine("Pressione Enter para continuar ...");
        Console.ReadKey();
        break;
        case "5":
        Console.WriteLine("=== Lista de clientes =====");
            foreach (var cliente in clientesArquivo)
    {
        Console.WriteLine($"ID: {cliente.Id} - Nome: {cliente.Nome} - Telefone: {cliente.Telefone}");
    }

    Console.WriteLine("Digite o ID do cliente que deseja apagar:");
    var idApagar = Console.ReadLine();

    if (!int.TryParse(idApagar, out int id))
    {
        Console.WriteLine("ID inválido!");
        Thread.Sleep(1000);
        break;
    }

    var clienteApagar = PostgresPersistencia.Buscar(id);

    if (clienteApagar == null)
    {
        Console.WriteLine("Cliente não encontrado!");
        Thread.Sleep(1000);
        break;
    }

    PostgresPersistencia.Apagar(id);
    Console.WriteLine("Cliente apagado com sucesso!");
    Thread.Sleep(1000);
        break;
        default:
            sair = true;
            break;
    }

    if(sair) break;
}